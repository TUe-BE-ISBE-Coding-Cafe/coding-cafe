...
{#if variant === 'upcoming'}
	...
	>
		...
		<div class="flex h-full flex-col pl-2">
			<h2 class="pt-2 text-2xl text-gray-800">{event.title}</h2>
			{#if event.subtitle}<h3 class="text-sm text-gray-600">{event.subtitle}</h3>{/if}
			{#if event.tags}
				<div class="flex flex-wrap gap-1 pb-2">
					{#each event.tags as tag}
						<span class="px-1 text-xs font-medium text-[#C81919]">#{tag}</span>
					{/each}
				</div>
			{/if}
			{#if event.description}<h3 class="text-sm text-black">{event.description}</h3>{/if}
		</div>
...
{:else}
	...
	>
		...
		<div class="flex h-full flex-col pl-2">
			<h2 class="text-2xl text-gray-800">{event.title}</h2>
			{#if event.subtitle}<h3 class="text-sm text-gray-600">{event.subtitle}</h3>{/if}
			{#if event.tags}
				<div class="flex flex-wrap gap-1">
					{#each event.tags as tag}
						<span class="px-1 text-xs font-medium text-red-700">#{tag}</span>
					{/each}
				</div>
			{/if}
			{#if event.description}<h3 class="text-sm text-gray-600">{event.description}</h3>{/if}
