<script>
	const githubUrl = 'https://github.com/ISBE-TUe/coding-cafe';
	const zenodoUrl = 'https://zenodo.org/communities/tue_be_isbe_codingcafe';
	const email = 'o.balaban@tue.nl';
	const creatorUrl = 'https://www.ozgunbalaban.com';
</script>

<footer class="mt-auto w-full bg-[#C81919] py-1 text-white">
	<div class="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 px-4 sm:flex-row">
		<!-- Left: TU/e logo -->
		<div class="shrink-0 bg-[#FFFFFF]">
			<a href="https://www.tue.nl" target="_blank" aria-label="TU/e homepage">
				<img src="/images/tuelogo.svg" alt="TU/e Logo" class="h-10 sm:h-12" />
			</a>
		</div>

		<!-- Center: ISBE text -->
		<div class="text-center text-xl font-light">
			<a href="https://isbe.bwk.tue.nl" class="block hover:underline">
				Information Systems in the Built Environment (ISBE)
			</a>
		</div>

		<!-- Right: Icons and Email -->
		<div class="flex items-center gap-6 text-sm">
			<a
				href="https://teams.microsoft.com/l/team/19%3ATfehyuNclRvZxP_XBBLUaPcVK0L_aHdbfs7NNHqcsqQ1%40thread.tacv2/conversations?groupId=81d0985d-b794-4f0e-8003-f7265709d8ab&tenantId=cc7df247-60ce-4a0f-9d75-704cf60efc64"
				target="_blank"
				aria-label="BE Coding Cafe Teams"
			>
				<img src="/images/teams_logo.svg" alt="BE Coding Cafe Teams" class="h-6" />
			</a>
			<a href={githubUrl} target="_blank" aria-label="GitHub">
				<svg class="h-6 w-6 fill-white hover:fill-gray-300" viewBox="0 0 24 24">
					<path
						d="M12 0.3a12 12 0 00-3.8 23.4c0.6 0.1 0.8-0.3 0.8-0.6v-2.2c-3.3 0.7-4-1.6-4-1.6-0.6-1.4-1.3-1.8-1.3-1.8-1-0.7 0.1-0.7 0.1-0.7 1.1 0.1 1.7 1.1 1.7 1.1 1 1.7 2.6 1.2 3.2 0.9 0.1-0.7 0.4-1.2 0.7-1.4-2.6-0.3-5.3-1.3-5.3-5.8 0-1.3 0.5-2.3 1.2-3.2-0.1-0.3-0.5-1.5 0.1-3.1 0 0 1-0.3 3.3 1.2a11.6 11.6 0 016 0c2.3-1.5 3.3-1.2 3.3-1.2 0.6 1.6 0.2 2.8 0.1 3.1 0.7 0.9 1.2 1.9 1.2 3.2 0 4.5-2.7 5.5-5.3 5.8 0.4 0.3 0.8 1 0.8 2v3c0 0.3 0.2 0.7 0.8 0.6A12 12 0 0012 0.3"
					/>
				</svg>
			</a>
			<a href={zenodoUrl} target="_blank" aria-label="Zenodo Community">
				<img src="/images/zenodo-icon-blue.svg" alt="Zenodo" class="h-6" />
			</a>
			<div class="flex items-center gap-3 text-left leading-tight">
				<p class="whitespace-nowrap text-sm text-white">
					Contact: <span class="font-medium">Özgün Balaban</span>
				</p>
				<div>
					<a href={'mailto:' + email} class="block text-xs underline hover:text-gray-300 sm:text-sm">
						{email}
					</a>
					<a href={creatorUrl} target="_blank" rel="noreferrer" class="block text-xs underline hover:text-gray-300 sm:text-sm">
						www.ozgunbalaban.com
					</a>
				</div>
			</div>
		</div>
	</div>
</footer>
